...
// List of repositories (or fetch dynamically if you want)
const repos = [
  "bugtracker",
  "Ananya",
  "Ashna-Technical",
...
async function fetchIssuesFromRepo(repoName) {
  try {
    const url = `https://api.github.com/repos/${GITHUB_ORG}/${repoName}/issues?state=open&per_page=100`;
    ...
    await Issue.updateOne(
      { github_issue_id: issue.id },
      {
        $set: {
          ...
          status: existing ? existing.status : "unreviewed",
